...
# Load weights
checkpoint = torch.load("ckpt/trace_finetune_mortality_nopretrain.pt", map_location=device)
...
print(f"✅ AUROC (No Pretraining): {auroc:.4f}")
print(f"✅ Accuracy (No Pretraining): {accuracy:.4f}")
...
plt.title("ROC Curve for Mortality Prediction (No Pretraining)")
plt.legend()
plt.grid()
plt.savefig("mortality_roc_curve_nopretrain.png")
plt.show()
