...
from models.TransformerTraceEncoder.trace_encoder import TRACEEncoder
...
for epoch in range(1, PretrainConfig.num_epochs + 1):
    ...
    for batch in dataloader:
        ...
        # Forward pass through encoder only (no contrastive branch)
        pred, _ = model(masked_data)

        # For consistency with reconstruction, project back to original dim       
        output, _ = model(masked_data)  # ✅ USE THIS INSTEAD

        # Only compute loss on masked positions
        loss = loss_fn(output[mask_indicator], data[mask_indicator])
    ...
    if epoch % PretrainConfig.save_every == 0:
        ...
        save_path = os.path.join(PretrainConfig.save_dir, f"trace_nocontrast_epoch{epoch}.pt")
...
plt.title('Pretraining Loss Curve (No Contrastive)')
plt.grid(True)
plt.show()
