# models/train_pretrain_shallow_encoder.py
...
# Override encoder depth (ABLATION)
shallow_num_layers = 2
...
model = TRACEModel(
    ...
    num_layers=shallow_num_layers,  # ⬅️ Modified here
...
for epoch in range(1, PretrainConfig.num_epochs + 1):
    ...
    for batch in dataloader:
        ...
        pred, contrastive_loss = model(masked_data)

        loss = loss_fn(pred[mask_indicator], data[mask_indicator]) + contrastive_loss
    ...
    if epoch % PretrainConfig.save_every == 0:
        ...
        save_path = os.path.join(PretrainConfig.save_dir, f"trace_pretrain_shallow_epoch{epoch}.pt")
...
plt.title('Pretraining Loss Curve (Shallow Encoder)')
plt.grid(True)
plt.show()
